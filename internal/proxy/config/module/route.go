...
func (m *RouteModule) applyDefaultFragments(opts *option.Options) error {
	...
	// 片段 2: NTP 直连
	rules = append(rules, map[string]any{"protocol": []string{"ntp"}, "outbound": TagDirect})
	...
	rules = append(rules, map[string]any{"ip_cidr": []string{"223.5.5.5/32", "223.6.6.6/32", "2400:3200::/32"}, "outbound": TagDirect})
	...
	rules = append(rules, map[string]any{"rule_set": []string{"geosite-ads", "anti-ad"}, "outbound": TagBlock})
	...
	rules = append(rules, map[string]any{"rule_set": []string{"geosite-apple"}, "outbound": TagDirect})
	...
	rules = append(rules, map[string]any{"rule_set": []string{"geosite-cn", "geoip-cn"}, "outbound": TagDirect})
