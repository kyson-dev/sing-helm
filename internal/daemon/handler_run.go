...
func (d *Daemon) handleRun(ctx context.Context, payload map[string]any) ipc.CommandResult {
	...
	if err := engine.BuildConfig(platform.Get().RawConfigFile, &runops); err != nil {
		return ipc.CommandResult{Status: "error", Error: fmt.Errorf("failed to build config: %w", err).Error()}
	}

	// 2. 启动 sing-box 服务
	svc := d.newService()
	rawPath := platform.Get().RawConfigFile
...
func (d *Daemon) applyRunOptions(ctx context.Context, state *model.RuntimeState) error {
	...
	backupPath, _ := backupConfig(platform.Get().RawConfigFile)
	if err := engine.BuildConfig(platform.Get().RawConfigFile, &state.RunOptions); err != nil {
		return err
	}
	if d.service == nil {
	...
	if err := d.service.ReloadFromFile(ctx, platform.Get().RawConfigFile); err != nil {
		...
		if errors.As(err, &reloadErr) && reloadErr.Stage == engine.ReloadStageStart {
			if backupPath != "" {
				if retryErr := d.service.StartFromFile(ctx, backupPath); retryErr == nil {
					if restoreErr := restoreConfig(backupPath, platform.Get().RawConfigFile); restoreErr != nil {
						return restoreErr
					}
					d.setRunning(true)
