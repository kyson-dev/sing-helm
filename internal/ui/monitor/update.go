...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "up":
            if m.Expanded {
                if m.CursorNode > 0 { m.CursorNode-- }
            } else {
                if m.CursorGroup > 0 { m.CursorGroup-- }
            }
        case "down":
            if m.Expanded {
                if m.CursorNode < len(m.ExpandedList)-1 { m.CursorNode++ }
            } else {
                if m.CursorGroup < len(m.Groups)-1 { m.CursorGroup++ }
            }
		// --- 展开/收起/选择逻辑 ---
        case "enter":
            if !m.Expanded {
