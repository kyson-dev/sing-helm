...
git clone https://github.com/kyson/homebrew-sing-helm.git "${WORK_DIR}"
cd "${WORK_DIR}"
...
class SingHelm < Formula
  ...
  homepage "https://github.com/kyson/sing-helm"
  version "${VERSION}"
  
  if Hardware::CPU.arm?
    url "https://github.com/kyson/sing-helm/releases/download/v${VERSION}/sing-helm-darwin-arm64"
    sha256 "${ARM64_SHA}"
  else
    url "https://github.com/kyson/sing-helm/releases/download/v${VERSION}/sing-helm-darwin-amd64"
    sha256 "${AMD64_SHA}"
  end
...
git push "https://x-access-token:${GITHUB_TOKEN}@github.com/kyson/homebrew-sing-helm.git" main
